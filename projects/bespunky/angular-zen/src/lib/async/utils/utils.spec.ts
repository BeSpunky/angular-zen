...
describe('promiseLater()', () =>
{
    ...
    it('should return a `resolve()` method that acts on the returned promise', () =>
    {
        const later = promiseLater();

        later.promise.then((value) => expect(value).toBe('resolved'));

        later.resolve('resolved');
    });

    it('should return a `reject()` method that acts on the returned promise', () =>
    {
        const later = promiseLater();

        later.promise.then(() => void 0, (reason) => expect(reason).toBe('rejected'));
