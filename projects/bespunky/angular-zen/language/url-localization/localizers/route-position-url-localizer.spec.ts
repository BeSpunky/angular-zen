...
import { DeepRoutePath, DeepRouteSegments, DeepRouteSegmentsNoRoot, setupUrlLocalizerTest } from '@bespunky/angular-zen/language/testing';
...
describe('RoutePositionUrlLocalizer', () =>
{
    ...
    function setup(testedRoute: string, langPosition: number)
    {
    ...
    function manipulateUrl(testedRoute: string, call: 'localize' | 'delocalize')
    {
        const initialSegments = urlReflection.routeSegmentsOf(testedRoute);
        const modifiedUrl = call === 'localize' ? localizer.localize('en') : localizer.delocalize();
    ...
    {
        await setup(config.testedRoute, config.langPosition);

        // Perform localization or delocalization
        const { initialSegments, modifiedSegments } = manipulateUrl(config.testedRoute, config.call);
    ...
    async function testNoChange(config: SimpleTestConfig)
    {
        ...
        expect(initialSegments).toEqual(modifiedSegments);
    }
    ...
    describe('localize', () =>
    {
        ...
        {
            it('return an unchanged url if a language segment is already present', () => testLocalization({
        ...
        {
            it('should return an unchanged url', () => testNoChange({
                testedRoute : `/en${DeepRoutePath}`,
                call        : 'localize',
                langPosition: 1
            }));
    ...
    describe('delocalize', () =>
    {
        describe('for positive positions', () =>
        {
            it('return an unchanged url if a language segment is not present', () => testLocalization({
                testedRoute          : DeepRoutePath,
                call                 : 'delocalize',
                langPosition         : 1,
                expectedLangSegmentAt: 0,
        ...
        describe('for zero positions', () =>
        {
            xit('should return an unchanged url', async () => 
            {                      
                await setup(DeepRoutePath, 0);

                const { initialSegments, modifiedSegments } = manipulateUrl(DeepRoutePath, 'localize');
